...
const primaryLinks = [
  ...
  { href: "/cbam-calculator/actual-data", label: "Actual / Complex Goods" },
  { href: "/cbam-calculator/electricity", label: "CBAM Electricity" },
  { href: "/cbam-calculator/bulk", label: "Official Bulk CSV" },
...
export default function MobileNav() {
  ...
  // Prevent the page behind the open menu from scrolling and support Escape.
  useEffect(() => {
    if (!open) return;
      ...
      {open && (
        <>
          <button
            ...
            onClick={closeMenu}
            className="fixed inset-x-0 bottom-0 top-16 z-[55] bg-slate-950/25 sm:top-20"
          />

          <div
            id="mobile-navigation-panel"
            className="fixed inset-x-0 top-16 z-[60] max-h-[calc(100vh-4rem)] overflow-y-auto overscroll-contain border-b border-slate-200 bg-white shadow-2xl sm:top-20 sm:max-h-[calc(100vh-5rem)]"
          >
            <div className="mx-auto w-full max-w-7xl px-4 py-5 sm:px-6 sm:py-6">
              <section className="rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
                <p className="px-1 pb-3 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">
                  Calculators
                </p>
                ...
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  <Link
                    ...
                    onClick={closeMenu}
                    className="rounded-xl bg-slate-900 px-4 py-3.5 text-center text-sm font-black text-white transition-colors hover:bg-slate-800"
                  >
                  ...
                  <Link
                    ...
                    onClick={closeMenu}
                    className="rounded-xl bg-emerald-600 px-4 py-3.5 text-center text-sm font-black text-white transition-colors hover:bg-emerald-500"
                  >
              ...
              <section className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 sm:p-4">
                <p className="px-1 pb-2 text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">
                  Learning guides
                </p>
                ...
                <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                  {learningLinks.map((item) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={closeMenu}
                      className="rounded-lg px-3 py-2.5 text-sm font-semibold text-slate-600 transition-colors hover:bg-white hover:text-emerald-700"
                    >
                      {item.label}
                    </Link>
                  ))}
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 border-t border-slate-200 pt-4">
                <Link
                  href="/privacy-policy"
                  onClick={closeMenu}
                  className="rounded-lg px-3 py-2.5 text-center text-xs font-bold text-slate-500 hover:bg-slate-50 hover:text-slate-800"
                >
                  Privacy
                </Link>
                <Link
