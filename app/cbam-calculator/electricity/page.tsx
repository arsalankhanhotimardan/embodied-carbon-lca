...
const ELECTRICITY_CBAM_VERSION = "CBAM-Electricity-V1";
...
export default function ElectricityCbamPage() {
  ...
  const anonymousWorkspaceSnapshot = useMemo(
    () => ({
      ...
      appVersion: ELECTRICITY_CBAM_VERSION,
      country,
      year,
      mwh,
      mode,
  ...
  const restoreAnonymousWorkspace = (raw: any) => {
    ...
    const restoredYear = Number(raw.year);
    if (Number.isInteger(restoredYear) && restoredYear >= 2026 && restoredYear <= 2100) {
      setYear(restoredYear);
    }
    setMwh(typeof raw.mwh === "string" ? raw.mwh : String(raw.mwh ?? ""));
    setMode(raw.mode === "actual" ? "actual" : "default");
    setEf(typeof raw.ef === "string" ? raw.ef : String(raw.ef ?? ""));
    setPpa(Boolean(raw.ppa));
    setConnection(Boolean(raw.connection));
    setNomination(Boolean(raw.nomination));
    setVerifier(Boolean(raw.verifier));
    setSelectedPriceKey(
  ...
  return <main className="mx-auto min-w-0 max-w-5xl overflow-x-hidden px-3 py-6 sm:px-6 sm:py-10">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0"><div className="text-xs font-black uppercase tracking-widest text-blue-700">Dedicated sector methodology</div><h1 className="mt-2 break-words text-2xl font-black text-slate-950 sm:text-4xl">CBAM electricity calculator</h1><p className="mt-3 max-w-3xl text-slate-600">Electricity is measured in MWh. No demonstration country, MWh quantity, emission factor or certificate price is preloaded.</p></div>
      <Link href="/cbam-calculator" className="w-full rounded-xl border border-slate-300 px-4 py-2.5 text-center font-bold sm:w-auto">← Main calculator</Link>
    </div>
    <AnonymousWorkspacePanel
      toolId="cbam-electricity"
      toolLabel="CBAM electricity"
      toolVersion={`${ELECTRICITY_CBAM_VERSION} · ${CBAM_WORKSPACE_VERSION}`}
      snapshot={anonymousWorkspaceSnapshot}
      onRestore={restoreAnonymousWorkspace}
      defaultSaveName={country ? `${country} electricity CBAM` : "CBAM electricity project"}
      className="mt-6"
    />
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      <section className="min-w-0 space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-2 rounded-xl bg-slate-100 p-1 sm:grid-cols-2"><button type="button" onClick={() => setMode("default")} className={`rounded-lg p-2 font-black ${mode === "default" ? "bg-white shadow" : ""}`}>Official default EF</button><button type="button" onClick={() => setMode("actual")} className={`rounded-lg p-2 font-black ${mode === "actual" ? "bg-white shadow" : ""}`}>Actual electricity</button></div>
