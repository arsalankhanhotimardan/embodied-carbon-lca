...
const CSI_DIVISIONS = [
    "Div 03: Concrete", "Div 04: Masonry", "Div 05: Metals", "Div 06: Wood, Plastics, and Composites",
    "Div 07: Thermal and Moisture Protection", "Div 08: Openings", "Div 09: Finishes", "Div 10-49: Other"
];

export default function LcaCalculator() {
  ...
  const generateNativePDF = () => {
    ...
    try {
        ...
        // ENTERPRISE FIX: Added Module D to PDF Output
  ...
  return (
    <div className="w-full bg-white rounded-sm shadow-2xl overflow-hidden border border-gray-200 relative">
      ...
      {showRevitModal && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 h-screen w-screen">
...
public async Task SyncToSaaS(object revitScheduleData)
{
    ...
    if(response.IsSuccessStatusCode) {
      ...
      {pendingReconciliation && (
        <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-sm flex items-center justify-center p-4 h-screen w-screen overflow-hidden">
      ...
      {/* --- HEADER CONTROLS --- */}
      <div className="bg-slate-900 px-6 py-5 flex flex-wrap justify-between items-center gap-4 border-b border-slate-800">
      ...
      {pendingUpload ? (
        <div className="bg-slate-50 p-10 min-h-[500px] flex items-center justify-center">
        ...
        <>
          {hasData && (
            <div className="flex bg-slate-50 text-sm font-semibold text-slate-500 overflow-x-auto shadow-inner border-b border-slate-200">
          ...
          <div className="p-8 min-h-[500px] bg-slate-50">
                    ...
                    {isComparing ? (
                        <>
                            ...
                            <div className="flex gap-2">
                                ...
                                <button onClick={generateNativePDF} disabled={isDownloading} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 text-sm font-bold rounded shadow-sm transition-all disabled:opacity-50">
                                    {isDownloading ? 'Generating...' : 'Download Full PDF'}
                                </button>
                            </div>
                        </>
                    ) : (
                            ...
                            <div className="h-[400px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }} barSize={100}>
                                        ...
                                        <Tooltip formatter={(value: any) => `${Number(value).toLocaleString()} kg CO₂e`} cursor={{ fill: '#f8fafc' }} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                                ...
                                <div className="h-[400px] w-full">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={crossoverChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                            ...
                                            <Tooltip formatter={(value: any) => `${Number(value).toLocaleString()} kg CO₂e`} contentStyle={{ borderRadius: '8px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }} />
                                            <Legend wrapperStyle={{ paddingTop: '20px' }} />
